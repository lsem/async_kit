...
std::ostream& operator<<(std::ostream& os, const test_sample& s) {
    os << "test_sample(limit_num: " << s.limit_num << ", items_num: " << s.items_num
...
INSTANTIATE_TEST_SUITE_P(
    sequence_size_and_limit_combinations,
    bounded_async_foreach_p,
    ::testing::Values(test_sample{14, 100, 14}, test_sample{1, 10, 1}, test_sample{0, 100, 0}, test_sample{10, 5, 5}));
...
TEST(bounded_async_foreach, finished_callback_call_test) {
    ...
    bounded_async_foreach(
        ...
        [&](const std::string& item, auto done_cb) {
            ...
            async_sleep(ctx, random_time, [&n_running, done_cb = std::move(done_cb)] {
                n_running--;
                done_cb(std::error_code());
            });
        },
...
TEST(bounded_async_foreach, basic_randomized_test) {
    ...
    for (int run_n = 0; run_n < 5; run_n++) {
        ...
        EXPECT_EQ(n_running_max, std::min(limit_n, (int)items.size())) << "seed was :" << seed;
...
TEST(bounded_async_foreach, empty_collection_test) {
    ...
    bounded_async_foreach(
        1, input, [](auto element, auto done) { ASSERT_TRUE(false) << "not supposed to be called\n"; },
        [&](std::error_code ec) {
            EXPECT_FALSE(ec);
            finish_called = true;
        });
...
TEST(bounded_async_foreach, zero_limit_test) {
    ...
    bounded_async_foreach(
        0, input, [](auto element, auto done) { ASSERT_TRUE(false) << "not supposed to be called\n"; },
        [&](std::error_code ec) {
...
TEST(bounded_async_foreach, after_cancel_no_new_starts_test) {
    ...
    bounded_async_foreach(
        ...
        [&](int item, auto done_cb) {
            ...
            async_sleep(ctx, 300ms, [done_cb = std::move(done_cb)] { done_cb(std::error_code()); });
        },
        [&](std::error_code ec) {
        ...
        bounded_async_foreach(
            ...
            [&](int item, auto done_cb) {
                ...
                if (item == 3) {
                    ...
                    async_sleep(ctx, 300ms, [item, &item_done_calls, done_cb = std::move(done_cb)] {
...
TEST(bounded_async_foreach, finish_called_after_last_item_done__pending_jobs_fail) {
